...
class Activite
{
	...
	*/
	public function __construct($id, $timeStart=NULL, $nom=NULL, $descriptif=NULL, $duree=NULL, $lieu=NULL, $type=NULL, $placesLim=NULL, $prix=NULL, $idOwner=NULL, $points=NULL, $mustBeReserved=NULL, $debutReservation=NULL, $finReservation=NULL, $idRecurrente=NULL) {
		$this->_id = $id;
		if ($id==NULL)
		{	
		...
		else
		{
			...
			$this->_idCreateur = $data['idCreateur'];
		}
		$this->_deleted=false;
	}
	public function saveToDb(){
		...
		else if ($this->_id!=NULL && $database->count('activities', array("id" => $this->_id))) //Id non null et Existe en db, on update
		{
			...
			array("debutReservation" => $this->_debutReservation, "finReservation" => $this->_finReservation,
		...
		{
			$database->create('activities', array("idCreateur" => $_SESSION["id"], "debutReservation" => $this->_debutReservation, "finReservation" => $this->_finReservation, "id" => $this->_id, "mustBeReserved" => $this->_mustBeReserved, "time_start" => $this->_timeStart, "duree" => $this->_duree, "nom" => $this->_nom, "description" => $this->_descriptif, "type" => $this->_type, "lieu" => $this->_lieu, "points" => $this->_points, "prix" => $this->_prix, "capaciteMax" => $this->_placesLim, "idDirigeant" => $this->_idOwner,'idRecurrente' => $this->_idRecurrente));
			$this->setId($database->lastInsertId());
		} 
	}
	public function getNomByLang(){
		$nomAct=unserialize($this->_nom);
		$nom=current($nomAct);
