<?php
	...
	$count=$database->count("users", $where);
	if ($count>0 && (!isset($_POST["limit"]) || $count<=$_POST["limit"]))
	{
		?>
	...
	else
	{
		?>
		<div class="info message_block">
			<p>Il y a trop de résultat. Merci d'affiner votre recherche.</p>
		</div>
		<?php
	}
?>
<script type="text/javascript">
	...
	{
		$("#<?php echo htmlentities($input); ?>").val(name);
		$("#<?php echo htmlentities(str_replace("name", "id", $input)); ?>").val(id);
		$(".custom_table.saw").remove();
	}
</script>